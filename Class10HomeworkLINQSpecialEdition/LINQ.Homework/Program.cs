    ...
    class Program
    {
        ...
        static void Main(string[] args)
        {
            ...
            */
            ...
            List<Song> firstLetterA = Songs
                                            .Where(song => song.Name.ToLower()
                                            .StartsWith('a'))
                                            .ToList();
            Console.WriteLine($"- {firstLetterA.Count} songs");

            Console.WriteLine("2. How many artists end with letter 'a' (case insensitive)?");
            List<Artist> lastLetterA = Artists
                                                .Where(artist => artist.FullName.ToLower()
                                                .EndsWith('a'))
                                                .ToList();
            Console.WriteLine($"- {lastLetterA.Count} artists");
            ...
            var longestDurationSong = Songs
                                                ...
                                                .Select(sec => sec.Duration)
                                                .Max());
            Console.WriteLine($"- {longestDurationSong.Name} ({longestDurationSong.Duration}sec.)");

            Console.WriteLine("4. Whats the total Duration of all Songs?");
            int sumOfSongsDuration = Songs
                                            .Select(song => song.Duration)
                                            .Sum();
            Console.WriteLine($"{- sumOfSongsDuration} sec.");

            Console.WriteLine("5. How many albums have Songs longer than 300 seconds?");

            int  albumLongSong = Albums
                                            .Where(album => album.Songs
                                            .Any(song => song.Duration > 300))
                                            .Count();
            Console.WriteLine($"- {albumLongSong} albums");

            Console.WriteLine("6. Print the names of the artists, that have more than one album of PopRock genre.");
            List<string> popRockGenreAlbum = Artists
                                                    .Where(artist => artist.Albums
                                                    .Any(album => album.Genre == Genre.PopRock))
                                                    .Where(number => number.Albums.Count > 1)
                                                    .Select(name => name.FullName)
                                                    .ToList();
            foreach (var artist in popRockGenreAlbum)
            {
                Console.WriteLine($"- {artist}");
            }

            Console.WriteLine("7. Print the name of the album that has highest Average duration of a song.");
            var highestAverageDuration = Albums
            ...
            Console.WriteLine($"- {highestAverageDuration.Name} album");

            Console.WriteLine("8. How many characters has the song that has the shortest Duration?");
            Song shortestSong = Songs
                                             .FirstOrDefault(song => song.Duration == Songs
                                                .Select(sec => sec.Duration)
                                                .Min());

            int numOfchars = shortestSong.Name.Count();
            Console.WriteLine($"- {shortestSong.Name} has {numOfchars} characters");
            ...
            Album albumMostSongs = Albums
                                        ...
                                        .Max());
            Console.WriteLine($"- Name: {albumMostSongs.Name}  Genre: {albumMostSongs.Genre}");

            Console.WriteLine("10. Print the name of the artist that has most songs.");
            Artist artistMostSongs = Artists
                                            .FirstOrDefault(artist => artist.Albums
                                            .Select(album => album.Songs.Count).Sum() == Artists
                                            .Select(artistt => artistt.Albums
                                            .Select(album => album.Songs.Count).Sum())
                                            .Max());
            Console.WriteLine($"- {artistMostSongs.FullName}");

            Console.WriteLine("11. Print the type of the artist(SoloArtist/Band) that has most albums published before year 2000.");
            Artist artistBefore2000 = Artists
                                            .FirstOrDefault(artist => artist.Albums
                                            .Count(album => album.Year < 2000) == Artists
                                            .Select(artistt => artistt.Albums
                                            .Count(albumm => albumm.Year < 2000))
                                            .Max());
            Console.WriteLine($"- {artistBefore2000.FullName}");


            Console.WriteLine("12. Print the average song duration, of the album that has most songs.");
            double averageSongDuration = Albums
                                        .FirstOrDefault(album => album.Songs.Count == Albums
                                        .Select(x => x.Songs.Count).Max())
                                        .Songs
                                        .Select(song => song.Duration)
                                        .Average();
            ...
            int longestSongDuration = Albums
                                       .FirstOrDefault(album => album.Songs.Count == Albums
                                       .Select(x => x.Songs.Count).Min())
                                       .Songs.Select(song => song.Duration)
                                       .Max();
            ...
            Album mostSongsAndLetterA = Albums
                                        .FirstOrDefault(album => album.Songs
                                        .Where(song => song.Name.ToLower().Contains('a'))
                                        .Count() == Albums
                                        .Select(albumm => albumm.Songs
                                        .Where(songg => songg.Name.ToLower().Contains('a'))
                                        .Count())
                                        .Max());
            Console.WriteLine($"- {mostSongsAndLetterA.Name}");


            Console.WriteLine("15. Print the name of the artist that has most songs that end with letter 'd'.");
            
            Artist artistSongsLetterD = Artists
                                         .FirstOrDefault(artist => artist.Albums
                                         .Select(album => album.Songs
                                         .Where(song => song.Name.ToLower().EndsWith('d'))
                                         .Count())
                                         .Sum() == Artists
                                         .Select(art => art.Albums
                                         .Select(alb => alb.Songs
                                         .Where(sg => sg.Name.ToLower().EndsWith('d'))
                                         .Count())
                                         .Sum())
                                         .Max());
            Console.WriteLine($"- {artistSongsLetterD.FullName}");
